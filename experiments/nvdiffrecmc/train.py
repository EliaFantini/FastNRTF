...
def optimize_mesh(
    ...
    for it, target in enumerate(dataloader_train):
        ...
        metrics['init_mem'].append(round(torch.cuda.memory_allocated() * 1e-9, 4))
        ...
        if optimize_light:
            lgt.update_pdf()
        metrics['pre_render_mem'].append(round(torch.cuda.memory_allocated() * 1e-9, 4))
        ...
        metrics['post_backward_mem'].append(round(torch.cuda.memory_allocated() * 1e-9, 4))
        # ==============================================================================================
        ...
        metrics['iter_time'].append(time.time() - start)
        metrics['max_all_mem'].append(round(torch.cuda.max_memory_allocated() * 1e-9, 4))
        if it % 50 == 0:
    ...
    with open('{}/metrics_{}_{}.pkl'.format(script_dir, cur_datetime, pass_name), 'wb') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default="configs/nerfactor_drums.json", help='Config file')
